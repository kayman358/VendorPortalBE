...
//     }
        ...
        confirmedDate = getCurrentTimeStamp();
                 String sql2;
                 
    for (String spl1 : spl) {
        try {
            sql2 = "Update VendorInformation set status = 'Confirmed', confirmeddate =? where id =?";
             if(con == null){
            con = ds.getConnection();
             }
            stmt = con.prepareStatement(sql2);
            stmt.setString(1, confirmedDate);
            stmt.setString(2, spl1);
            stmt.executeUpdate();
            
            
            String sql3 = "Update VendorInformation set confirmedBy = ? where id = ?";
            if(con == null){
            con = ds.getConnection();
            }
